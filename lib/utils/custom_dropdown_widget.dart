...
class CustomDropdown extends StatelessWidget {
  ...
  RxList selectedValueList = [].obs;

  @override
  Widget build(BuildContext context) {
    selectedValueList.value = selectedValues ?? [];
    ...
    return Obx(() {
      ...
      if (selectionMode == SelectionMode.single) {
        buttonLabel = selectedValueList.value?.isNotEmpty == true
            ? displayItem!(selectedValueList.value!.first)
            : hintText;
      } else {
        // Multi-selection: show names of selected items or hintText
        buttonLabel = selectedValueList.value?.isNotEmpty == true
            ? selectedValueList.value!.map((e) => displayItem!(e)).join(', ')
            : hintText;
      }
      ...
      return DropdownButtonHideUnderline(
        child: DropdownButton2<dynamic>(
            ...
            value: selectionMode == SelectionMode.single
                ? (selectedValueList.value?.isNotEmpty == true
                    ? selectedValueList.value!.first
                    : null)
            ...
            items: items!.map((item) {
              return DropdownMenuItem<dynamic>(
                value: item,
                child: Obx(() {
                  // Wrap the content of DropdownMenuItem in Obx
                  final isSelected =
                      selectedValueList.value?.contains(item) ?? false;
                  return Row(
                    children: [
                      Expanded(
                        child: Text(
                      ...
                      if (isSelected &&
                        ...
                        Padding(
                          ...
                          child: Icon(
                            Icons.check,
                            color:
                                isSelected ? primaryWhite : Colors.transparent,
                            size: 20,
                          ),
                        ),
                    ],
              ...
              if (newValue != null) {
                if (selectionMode == SelectionMode.single) {
                  ...
                  final currentSelection =
                      List<dynamic>.from(selectedValueList.value ?? []);
